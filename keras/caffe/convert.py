...
from ..models import Graph, Sequential
...
def caffe_to_keras(prototext, caffemodel, network_type, phase='train', debug=False):
        '''
        ...
        if network_type == 'Sequential':
            model = create_model_sequential(layers, 0 if phase == 'train' else 1, tuple(config.input_dim[1:]), debug)
        elif network_type == 'Graph':
            model = create_model_graph(layers, 0 if phase == 'train' else 1, tuple(config.input_dim[1:]), debug)
        else:
            raise NameError('Unknown network type: %s' % network_type)
        ...
        load_weights(model, weights, network_type)

        return model

def create_model_sequential(layers, phase, input_dim, debug=False):
    '''
    layers:
        a list of all the layers in the mode
    phase:
        parameter to specify which network to extract: training or test
    input_dim:
        input  dimensions of the configuration (if model is in deploy mode)
    '''
    ...
    for layer_nb in network:
        ...
        if type_of_layer == 'concat':
            ...
            model.add(Activation('linear', name=name))
            ...
            if layer_nb == 0:
                if pad_h + pad_w > 0:
                    model.add(ZeroPadding2D(padding=(pad_h, pad_w), input_shape=input_dim))
                    model.add(Convolution2D(nb_filter, nb_row, nb_col, subsample=(stride_h, stride_w), name=name))
                else:
                    model.add(Convolution2D(nb_filter, nb_row, nb_col, subsample=(stride_h, stride_w), input_shape=input_dim, name=name))
            else:
                if pad_h + pad_w > 0:
                    model.add(ZeroPadding2D(padding=(pad_h, pad_w)))
                model.add(Convolution2D(nb_filter, nb_row, nb_col, subsample=(stride_h, stride_w), name=name))

        elif type_of_layer == 'dropout':
            prob = layer.dropout_param.dropout_ratio
            model.add(Dropout(prob, name=name))

        elif type_of_layer == 'flatten':
            model.add(Flatten(name=name))

        elif type_of_layer == 'innerproduct':
            output_dim = layer.inner_product_param.num_output
            if len(model.layers[-1].output_shape[1:]) > 1:
                model.add(Flatten())
            model.add(Dense(output_dim, name=name))
            ...
            model.add(LRN2D(alpha=alpha, k=k, beta=beta, n=n, name=name))
            ...
            if pad_h + pad_w > 0:
                model.add(ZeroPadding2D(padding=(pad_h, pad_w), name=name))
                input_layer_name = name + '_zeropadding'
            if(layer.pooling_param.pool == 0): # MAX pooling
                model.add(MaxPooling2D(pool_size=(kernel_h, kernel_w), name=name))

            elif(layer.pooling_param.pool == 1): # AVE pooling
                model.add(AveragePooling2D(pool_size=(kernel_h, kernel_w), strides=(stride_h, stride_w), name=name))
            else: # STOCHASTIC?
...
def load_weights(model, weights, network_type):
    if network_type == 'Sequential':
        for layer_nb in range(0, len(model.layers)):
            layer = model.layers[layer_nb]
            if weights.has_key(layer.name):
                model.layers[layer_nb].set_weights(weights[layer.name])
    else:
